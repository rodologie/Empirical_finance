---
title: "Empirical Finance: Tutorial - Week 2"
...
To download our data we will be using the `quantmod` package. This package connects us to our sources, using their public API's. The data extracted will be converted to an `xts` object.

-   What is an `xts` object? I s a class of objects in R for time series, where indices are dates.

A brief reference to `getSymbols` syntax:

-   `Symbols`: requires the ticker (i.e. standardized identifier) of our variable
-   `src`: requires de source (i.e. where to read the ticker/which API to connect)
...
Let's start of by computing daily simple and log return.

The *one-period* **net simple return** between time $t$ and $t + 1$ is given by:

$$
R_{t+1} - 1 = \frac{P_{t+1}}{P_{t}} -1 \Leftrightarrow R_{t} - 1 = \frac{P_{t}}{P_{t-1}} -1
$$

The *one-period* **log return** between time $t$ and $t + 1$ is given by:

$$
r_{t+1} = ln(R_{t+1}) = ln\left(\frac{P_{t+1}}{P_{t}}\right) = ln(P_{t+1}) - ln(P_{t})
$$
...
It may be tempting to merge all series and compute simple and log returns in a single operation. However, this approach overlooks a critical data-integrity issue: not all time series are observed continuously over time.

Both the S&P 500 Index and the GBP/USD exchange rate exhibit systematic breaks over weekends, whereas the US Federal Funds Rate is recorded for all calendar dates. Moreover, the Federal Funds Rate begins earlier in the sample, which would naturally lead to a `left_join` of the other two series onto it.

This operation introduces missing values on non-trading days. As a consequence, return calculations become distorted: a Monday observation is no longer compared to the preceding Friday, but instead to an `NA`, breaking the economic interpretation of returns and compromising downstream analysis.
...
```{r}
# --- building this joined dataset to visualize our argument
# test <- usffr %>%
#   left_join(sp500, by = "date") %>%
#   left_join(gbpusd, by = "date") %>%
#   arrange(date) %>%
#   mutate(
#     across(
...
Compute summary statistics like mean, median, standard deviation, skewness, kurtosis (including the robust ones) min, max, and first-order serial correlation, and report then in % per annum.
...
Plot the histogram of each return series.
...
**Robust Measures of Skewness & Kurtosis**
...
## a)
Sample 1000 draws from a Normal distribution with $\mu$ = 0 and $\sigma$ = 1. Plot the histogram of these draws.
...
We can add a density curve on top. However, to do so, we need to replace frequency counts with densities, on the Y axis.
```{r, warning=FALSE, message=FALSE}
ggplot(data = n_dist, aes(x = x)) +
  geom_histogram(
...
t_df5_df30 <- rbind(t_df5, t_df30)

# --- overlap the densities in a single plot 
plot_t_df5_df30_overlap <- ggplot(t_df5_df30, aes(x = x, color = df)) +
  geom_density(
    linewidth = 1,
    fill = NA
    ) + 
  scale_color_manual(
    values = c("5" = "#C44E52", "30" = "#55A868"),
    name = "degrees of freedom"
  ) +
  scale_x_continuous(
    breaks = seq(
      floor(min(t_df5_df30$x)),
      ceiling(max(t_df5_df30$x)),
      by = 1
    )
  ) +
  scale_y_continuous(
...
```{r}
# --- combine the two into one data frame
t_df5_df5eps0.5 <- rbind(t_df5, st_df5eps0.5)

# --- overlap the densities in a single plot 
plot_t_df5_df5eps0.5_overlap <- ggplot(t_df5_df5eps0.5, aes(x = x, color = sk)) +
  geom_density(
    linewidth = 1,
    fill = NA
    ) + 
  scale_color_manual(
    values = c("0" = "#C44E52", "0.5" = "#8172B3"),
    name = "skewness"
  ) +
  scale_x_continuous(
    breaks = seq(
      floor(min(t_df5_df5eps0.5$x)),
      ceiling(max(t_df5_df5eps0.5$x)),
      by = 1
    )
  ) +
  scale_y_continuous(
